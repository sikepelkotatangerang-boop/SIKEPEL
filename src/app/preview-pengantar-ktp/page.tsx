...
export default function PreviewFormulirKTPPage() {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      router.push('/form-surat/pengantar-ktp');
    }
  }, []);
  ...
  const loadPreview = async (data: any) => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/preview-pengantar-ktp-html', {
        method: 'POST',
        headers: {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      // Call API to process Formulir KTP (convert to PDF with ConvertAPI, upload to Storage, save to DB)
      const response = await fetch('/api/process-pengantar-ktp', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      a.href = url;
      a.download = `Formulir_KTP_${formData.nama?.replace(/\s+/g, '_') || 'Document'}_${Date.now()}.pdf`;
      ...
      alert(
        'Dokumen Formulir KTP berhasil dibuat!\n\n' +
        '✓ PDF telah diunduh\n' +
      ...
      console.error('Error processing Formulir KTP:', error);
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Preview Dokumen Formulir KTP</h1>
              ...
              <>
                ...
                <div className="preview-container">
                  <iframe
                    ...
                    title="Preview Surat Pengantar KTP"
                  />
                </div>
              </>
            )}
