...
import { ArrowLeft, Download, Edit, Loader2, CheckCircle, Heart } from 'lucide-react';
import { mockAuth } from '@/lib/mockData';

export default function PreviewSuamiIstriPage() {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      router.push('/form-surat/suami-istri');
    }
  }, [router]);
  ...
  const loadPreview = async (data: any) => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/preview-suami-istri-html', {
        method: 'POST',
        headers: {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      // Call API to process Suami Istri (convert to PDF with ConvertAPI, upload to Storage, save to DB)
      const response = await fetch('/api/process-suami-istri', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      a.href = url;
      a.download = `SK_Suami_Istri_${formData.nama_suami.replace(/\s+/g, '_')}_${Date.now()}.pdf`;
      ...
      sessionStorage.removeItem('suami_istri_preview_data');

      // Show success message
      alert(
        'Dokumen Surat Keterangan Suami Istri berhasil dibuat!\n\n' +
        '✓ PDF telah diunduh\n' +
          ...
          <div className="flex items-center space-x-4">
            ...
            <div>
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-gray-900">Preview Surat Keterangan Suami Istri</h1>
              ...
              <>
                ...
                <div className="preview-container">
                  <iframe
                    ...
                    title="Preview Surat Keterangan Suami Istri"
                  />
                </div>
              </>
            )}
