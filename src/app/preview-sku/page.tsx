...
function PreviewSKUContent() {
  ...
  const loadPreview = async (data: any) => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/preview-sku-html', {
        method: 'POST',
        headers: {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      // Call API to process SKU (convert to PDF with ConvertAPI, upload to Storage, save to DB)
      const response = await fetch('/api/process-sku', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      a.href = url;
      a.download = `SKU_${formData.nama_pemohon.replace(/\s+/g, '_')}_${Date.now()}.pdf`;
      ...
      sessionStorage.removeItem('skuFormData');

      // Show success message
      alert(
        'Dokumen SKU berhasil dibuat!\n\n' +
        '✓ PDF telah diunduh\n' +
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Preview Dokumen SKU</h1>
...
export default function PreviewSKUPage() {
  return (
