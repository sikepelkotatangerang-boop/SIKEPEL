...
export default function PreviewBelumRumahPage() {
  ...
  useEffect(() => {
    ...
    if (storedData) {
      ...
      router.push('/form-surat/belum-rumah');
    }
  }, []);
  ...
  const loadPreview = async (data: any) => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/preview-belum-rumah-html', {
        method: 'POST',
        headers: {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      // Call API to generate document with ConvertAPI
      const response = await fetch('/api/generate-belum-rumah', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      a.href = url;
      a.download = `Surat_Belum_Rumah_${formData.nama_pemohon.replace(/\s+/g, '_')}_${Date.now()}.pdf`;
      ...
      sessionStorage.removeItem('belum_rumah_preview_data');
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">Preview Surat Keterangan Belum Memiliki Rumah</h1>
              ...
              <>
                ...
                <div className="preview-container">
                  <iframe
                    ...
                    title="Preview Surat Belum Rumah"
                  />
                </div>
              </>
            )}
