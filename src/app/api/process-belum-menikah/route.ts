...
/**
 * API untuk memproses Surat Keterangan Belum Menikah:
 * 1. Generate DOCX dari template KETERANGANBELUMMENIKAH.docx
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('Processing Belum Menikah for user:', userId);
    ...
    // Load the template
    const templatePath = join(process.cwd(), 'public', 'template', 'KETERANGANBELUMMENIKAH.docx');
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Template file KETERANGANBELUMMENIKAH.docx tidak ditemukan. Hubungi administrator.' },
        { status: 500 }
      );
    }
    ...
    const templateData = {
      ...
      // Keperluan
      peruntukan: formData.peruntukan || 'keperluan yang bersangkutan',
    ...
    tempDocxPath = join(tmpdir(), `belum_menikah_${Date.now()}.docx`);
    ...
    try {
      ...
      tempPdfPath = join(tmpdir(), `belum_menikah_${Date.now()}.pdf`);
    ...
    // Upload to Supabase Storage (bucket: pdf_surat, folder: belum-menikah)
    const jenisSuratFolder = 'belum-menikah';
    ...
    // For database compatibility (use same column names as SKTM)
    ...
    try {
      ...
      const archiveValues = [
        ...
        'Belum Menikah',
        new Date(),
        `Surat Keterangan Belum Menikah untuk ${formData.peruntukan}`,
