...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log('Generating preview PDF with Puppeteer for Suami Istri...');

    // Generate HTML (call preview API)
    const htmlResponse = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'http://localhost:3000'}/api/preview-suami-istri-html`, {
      method: 'POST',
      headers: {
    ...
    return new NextResponse(pdfBuffer as any, {
      headers: {
        ...
        'Content-Disposition': 'inline; filename="preview-suami-istri.pdf"', // inline for preview
      },
    });
