...
/**
 * API untuk generate preview HTML Surat Keterangan Belum Menikah
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const templateData = {
      ...
      perkawinan: formData.perkawinan || 'Belum Kawin',
...
<head>
  ...
  <title>Preview Surat Keterangan Belum Menikah - ${data.nama_pemohon}</title>
  <style>
    * {
      margin: 0;
      padding: 0;
  ...
  <div class="preview-container">
    ...
    <div class="content">
      ...
      <table style="margin-left: 40px; margin-top: 15px;">
        ...
        <tr>
          <td>Alamat</td>
          <td>:</td>
          <td>${data.alamat} RT ${data.rt} RW ${data.rw}<br/>Kel. ${data.kelurahan} Kec. ${data.kecamatan} ${data.kota_kabupaten}.</td>
        </tr>
      </table>
      
      <p style="text-indent: 40px; margin-top: 15px;">Sesuai nama tersebut diatas Warga/Penduduk Kelurahan ${data.kelurahan}, menurut pengakuan yang bersangkutan dan keterangan Ketua RT ${data.rt} RW ${data.rw} ${data.pengantar_rt ? `(${data.pengantar_rt})` : ''} bahwa yang bersangkutan pada saat ini belum pernah menikah baik resmi maupun tidak resmi.</p>
      
      <p style="text-indent: 40px; margin-top: 10px;">Surat keterangan ini dikeluarkan dan dipergunakan untuk ${data.peruntukan || '{peruntukan}'}.</p>
    ...
    <div class="signature">
      <div class="signature-content">
        <p>${data.kota_kabupaten || 'Tangerang'}, ${data.tanggal_surat}</p>
